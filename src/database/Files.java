...
public class Files {
	 ...
	 *            Database connection
	 * @param email
	 *            Email of the user to retrieve its files
	...
	public static ArrayList<String[]> getFileNames(Connection c, String email) throws SQLException {
		...
		PreparedStatement preparedStatement = c
				.prepareStatement("SELECT * FROM (p2p.files JOIN p2p.users USING (user_id)) WHERE email = ?");
	 ...
	 *            Name of the file
	 * @param isPublic
	 *            Privacy of file (private or public)
	 * @return Inserted File ID
	 * @throws SQLException
	 */
	public static int insertNewFile(Connection c, String email, String name, boolean isPublic, long size)
			throws SQLException {
		PreparedStatement preparedStatement = c.prepareStatement(
				"INSERT INTO p2p.files(user_id, name, public, size) VALUES ((SELECT user_id FROM p2p.users WHERE email = ?), ?, ?, ?) RETURNING file_id");
		preparedStatement.setString(1, email);
